...
public class ConfiguracaoMouseListener implements ActionListener {
	...
	public void showSpeedMenu(ActionEvent e) {
		...
		String speed = (String) JOptionPane.showInputDialog((JMenuItem) e.getSource(), "Escolha a velocidade:",
				"Configurar Velocidade", JOptionPane.QUESTION_MESSAGE, null, speeds,
				this.backgroundanimator.getAnimationSpeedString());
		if (speed != null) {
			switch (speed) {
			case "Lento":
	...
	public void showColorMenu(ActionEvent e) {
		Color color1 = JColorChooser.showDialog((JMenuItem) e.getSource(), "Escolha a primeira cor",
				this.backgroundanimator.getPanel().getBackground());
		if (this.backgroundanimator.getCurrentPattern() != "Cor Sólida") {
			Color color2 = JColorChooser.showDialog((JMenuItem) e.getSource(), "Escolha a segunda cor",
					this.backgroundanimator.getPanel().getBackground());
	...
	public void showPatternMenu(ActionEvent e) {
		String[] patterns = { "Gradiente", "Transição de Cores", "Padrão Geométrico", "Linhas Aleatórias",
				"Cor Sólida" };
		String pattern = (String) JOptionPane.showInputDialog((JMenuItem) e.getSource(), "Escolha um padrão:",
				"Configurar Padrão", JOptionPane.QUESTION_MESSAGE, null, patterns,
				this.backgroundanimator.getCurrentPattern());
		if (pattern != null) {
